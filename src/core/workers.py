...
class PostProcessorWorker:
    ...
    def _download_single_file(self, file_info, target_folder_path, post_page_url, original_post_id_for_log, skip_event,
        ...
        if forced_filename_override:
            ...
            if self.manga_mode_active:
                ...
                if is_url_like or is_too_long:
                    ...
                    try:
                        parsed_real_url = urlparse(file_url)
                        real_ext = os.path.splitext(parsed_real_url.path)[1]
                        if real_ext and len(real_ext) <= 10:
                            ext = real_ext
                        else:
                            ext = ".file"
                    except Exception:
        ...
        if download_successful_flag:
            ...
            try:
                ...
                file_level_match = known_name_match_found
                post_level_match = False
                skip_ai_for_unknowns = getattr(self, 'commands', {}).get('handle_unknown', False)
                ...
                if getattr(self, 'visual_sort_active', False) and (is_image(final_save_path) or is_video(final_save_path)) and not post_level_match and not file_level_match and not skip_ai_for_unknowns:
                    try:
                        ...
                        if os.path.exists(model_path) and os.path.exists(csv_path):
                            ...
                            sort_result = sorter.process_image(final_save_path, candidate_chars=candidate_chars_for_ai)
                            ...
                            if not character_name and candidate_chars_for_ai and len(candidate_chars_for_ai) == 2:
                                top_3 = sort_result.get("top_3_chars", [])
                                ai_preference = None
                                
                                for ai_guess, score in top_3:
                                    ai_guess_norm = ai_guess.lower().replace("_", " ").strip()
                                    for candidate in candidate_chars_for_ai:
                                        for known_obj in self.known_names:
                                            if known_obj.get("name") == candidate:
                                                all_terms = [candidate] + known_obj.get("aliases", [])
                                                normalized_terms = [t.lower().replace("_", " ").strip() for t in all_terms if t]
                                                if ai_guess in known_obj.get("aliases", []) or ai_guess_norm in normalized_terms:
                                                    ai_preference = candidate
                                                    break
                                        if ai_preference: break
                                    if ai_preference: break
                                
                                if ai_preference:
                                    other_candidate = candidate_chars_for_ai[0] if candidate_chars_for_ai[1] == ai_preference else candidate_chars_for_ai[1]
                                    
                                    character_name = other_candidate
                                    sort_result["best_char"] = character_name
                                    sort_result["reason"] = f"POE fallback: AI failed to confirm {ai_preference}, defaulting to {other_candidate}"
                                else:
                                    character_name = candidate_chars_for_ai[1]
                                    sort_result["best_char"] = character_name
                                    sort_result["reason"] = f"POE fallback: AI hallucinated, defaulting to {character_name}"

                            folder_name = character_name.title() if character_name else "Unknown"
                            folder_name = clean_folder_name(folder_name) 
                    
                            is_in_known_txt = False
                            if character_name:
                                char_name_normalized = character_name.lower().replace("_", " ").strip()
                                
                                for known_obj in self.known_names:
                                    primary_name = known_obj.get("name", "")
                                    aliases = known_obj.get("aliases", [])
                                    
                                    all_known_terms = [primary_name] + aliases
                                    normalized_terms = [term.lower().replace("_", " ").strip() for term in all_known_terms if term]
                                    
                                    if char_name_normalized in normalized_terms:
                                        is_in_known_txt = True
                                        folder_name = clean_folder_name(primary_name)
                                        break

                            actual_base_dir = self.override_output_dir if self.override_output_dir else self.download_root

                            if self.use_subfolders:
                                if is_in_known_txt:
                                    base_target_dir = actual_base_dir
                                    log_path_prefix = ""
                                else:
                                    base_target_dir = os.path.join(actual_base_dir, "Visually Sorted")
                                    log_path_prefix = "Visually Sorted/"
                            else:
                                base_target_dir = effective_save_folder
                                log_path_prefix = ""
                                
                            new_folder_path = os.path.join(base_target_dir, folder_name)
                            ...
                            if character_name:
                                if "fallback" in reason:
                                    self.logger(f"   👁️ AI Sorted '{final_filename_on_disk}' -> {log_path_prefix}{folder_name} (Fallback!)\n   (Reason: {reason})")
                                else:
                                    self.logger(f"   👁️ AI Sorted '{final_filename_on_disk}' -> {log_path_prefix}{folder_name}\n   (Reason: {reason})\n   (Top3: {top_3_str})")
                            else:
                                self.logger(f"   👁️ AI Failed: Sent '{final_filename_on_disk}' -> {log_path_prefix}Unknown\n   (Reason: {reason})")
        ...
        if self.manga_filename_style == STYLE_POST_TITLE:
            ...
            try:
                ...
                format_values = {
                    ...
                    'service': self.service,
                    'title': str(self.post.get('title', '')),
                    'name': robust_clean_name(post_title),
                    'added': format_date(added_date or published_date),
                    'published': format_date(published_date),
                    'edited': format_date(edited_date or published_date)
                }
    ...
    def process(self):
        ...
        try:
            ...
            unique_paths_for_count = set()
            if post_main_file_info and isinstance(post_main_file_info, dict) and post_main_file_info.get('path'):
                unique_paths_for_count.add(post_main_file_info['path'])
            
            for att_info in post_attachments:
                if isinstance(att_info, dict) and att_info.get('path'):
                    unique_paths_for_count.add(att_info['path'])
            
            num_potential_files_in_post = len(unique_paths_for_count)
            if num_potential_files_in_post == 0:
                    ...
                    try:
                        ...
                        if file_extension == 'pdf':
                            ...
                            success = create_individual_pdf(
                                ...
                                font_path=font_path,
                                add_info_page=self.add_info_in_pdf,
                                logger=self.logger
                            )
            ...
            if not self.extract_links_only and self.use_subfolders:
                ...
                if primary_char_filter_for_folder:
                    ...
                    derived_folders_from_title_via_known_txt, title_unique_count = match_folders_from_title(
                    ...
                    if valid_derived_folders_from_title_known_txt:
                        if len(valid_derived_folders_from_title_known_txt) == 1:
                            known_name_match_found = True
                            ...
                            if title_unique_count > 1 and num_potential_files_in_post < len(valid_derived_folders_from_title_known_txt):
                                known_name_match_found = True
                                first_match = valid_derived_folders_from_title_known_txt[0]
                                base_folder_names_for_post_content.append(first_match)
                                self.logger(f"   Base folder name (Group Photo Logic: {num_potential_files_in_post} file(s) vs {len(valid_derived_folders_from_title_known_txt)} characters): '{first_match}'")
                            else:
                                self.logger(f"   ⚠️ Title is ambiguous for the overall post ({', '.join(valid_derived_folders_from_title_known_txt)}). Deferring to file-level AI sort.")
                        ...
                        for file_info_for_fallback in all_files_from_post_api_for_char_check:
                            ...
                            matched_folders_from_filename, _ = match_folders_from_filename_enhanced(
                                filename_for_fallback, self.known_names, effective_unwanted_keywords_for_folder_naming, post_title=post_title
                            )

                            if matched_folders_from_filename:
                                if len(matched_folders_from_filename) == 1:
                                    known_name_match_found = True
                                    ...
                                    self.logger(f"   ⚠️ Filename '{filename_for_fallback}' is ambiguous ({', '.join(matched_folders_from_filename)}). Deferring folder naming to AI.")
            ...
            if not self.extract_links_only and should_create_post_subfolder:
                ...
                if self.use_date_prefix_for_subfolder:
                    published_date_str = self.post.get('published') or self.post.get('added')
                    post_id_for_format = str(self.post.get('id', ''))
            ...
            with ThreadPoolExecutor(max_workers=self.num_file_threads, thread_name_prefix=f'P{post_id}File_') as file_pool:
                futures_list = []
                has_logged_ambiguous_title = False
                for file_idx, file_info_to_dl in enumerate(files_to_download_info_list):
                    ...
                    if not file_is_candidate_by_char_filter_scope:
                        ...
                        raw_file_path = str(file_info.get('path', '') or file_info.get('url', ''))
                        
                        hash_match = re.search(r'([a-fA-F0-9]{64})', raw_file_path)
                        kemono_hash = hash_match.group(1) if hash_match else "unknown_hash"
                        
                        self.logger(f"   -> Skip File (Char Filter Scope '{self.char_filter_scope}'): '{current_api_original_filename}' no match. (hash - {kemono_hash})")
                    ...
                    if self.use_subfolders:
                        title_folders, title_unique_count = match_folders_from_title(post_title, self.known_names, effective_unwanted_keywords_for_folder_naming)
                        filename_folders, file_unique_count = match_folders_from_filename_enhanced(current_api_original_filename, self.known_names, effective_unwanted_keywords_for_folder_naming, post_title=post_title)

                        if self.char_filter_scope in [CHAR_SCOPE_FILES, CHAR_SCOPE_BOTH] and filename_folders:
                            if len(filename_folders) == 1:
                                base_folder_for_this_file = filename_folders[0]
                                known_name_match_found_for_this_file = True
                            else:
                                if not has_logged_ambiguous_title:
                                    self.logger(f"   ⚠️ Filename is ambiguous between ({', '.join(filename_folders)}). Forwarding to AI Visual Sort.")
                                    has_logged_ambiguous_title = True
                                base_folder_for_this_file = ""
                                known_name_match_found_for_this_file = False
                            ...
                            if len(title_folders) == 1:
                                ...
                                if title_unique_count > 1 and num_potential_files_in_post < len(title_folders):
                                    ...
                                    known_name_match_found_for_this_file = True
                                    if not has_logged_ambiguous_title:
                                        self.logger(f"   ℹ️ Group Photo Logic: Assigning to '{title_folders[0]}' ({num_potential_files_in_post} file(s) vs {len(title_folders)} chars in title).")
                                        has_logged_ambiguous_title = True
                                else:
                                    if not has_logged_ambiguous_title:
                                        self.logger(f"   ⚠️ Title is ambiguous between ({', '.join(title_folders)}). Forwarding to AI Visual Sort.")
                                        has_logged_ambiguous_title = True
                                    base_folder_for_this_file = ""
                                    known_name_match_found_for_this_file = False
                                    candidate_chars_for_ai = title_folders
                                
                        elif filename_folders:
                            if len(filename_folders) == 1:
                                base_folder_for_this_file = filename_folders[0]
                                known_name_match_found_for_this_file = True
                            else:
                        ...
                        if not known_name_match_found_for_this_file:
                            if getattr(self, 'visual_sort_active', False):
                                base_folder_for_this_file = ""
                            elif self.handle_unknown_mode: 
